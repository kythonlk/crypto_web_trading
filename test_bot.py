...
def run_live_public_api_test():
    ...
    print(
        f"      Fetched {len (candles )} candles. Latest Close: ${candles .iloc [-1 ]['close']:,.2f}"
    )
    ...
    for pos in positions:
        print(
            f"      -> Ticket #{pos .ticket }: {pos .order_type } {pos .volume } @ {pos .open_price :.2f} | SL: {pos .sl :.2f} | TP: {pos .tp :.2f}"
        )
    ...
    print(
        f"      Updated SL: {updated_pos .sl :.2f} (Entry was: {pos_item .open_price :.2f})"
    )
    assert (
        updated_pos.sl >= pos_item.open_price
    ), "Breakeven SL should be at or above entry price!"
    print(
        "      [PASSED] Stop Loss successfully moved to Breakeven to guarantee risk-free trade!"
    )
    ...
    print(
        f"      Final Balance: ${new_balance :,.2f} | Realized Profit: ${new_balance -10000.0 :+,.2f}"
    )
